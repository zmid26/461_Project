...
for url in urls:
    ...
    printed_result = str(
        subprocess.run(
            ["./run", "score", "test_suite/newTest.txt"], stdout=subprocess.PIPE
        ).stdout
    )
    ...
    if (
        net_score <= 1 and net_score >= 0
    ):  # if the net_score is valid, add it as a passed test
...
print("Coverage: n/a%")
print(
    "%i/%i test cases passed. n/a%% line coverage achieved"
    % (len(net_scores), len(urls))
)
